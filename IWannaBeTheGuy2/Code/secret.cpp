    ...
    ui(new Ui::Secret)
{
    ...
    backgroundStart=new QPixmap(":/Images/MenuStart.png");
    ...
    QPixmap pixmap(":/Images/Jouer.png");
    ...
    pixmap.load(":/Images/Quitter.png");
    ...
    pixmap.load(":/Images/credits.png");
    ...
    pixmap.load(":/Images/commandeButton.png");
    ...
    for(int i=0;i<14;i++)
    {
        for(int j=0;j<16;j++)
        {
            if(itsLevel[j][i]->getValue()=='Z' && (itsPerson->getRect().x()>(itsLevel[j][i]->getRect().x()-45) && itsPerson->getRect().x()<(itsLevel[j][i]->getRect().x()+90)) && itsPerson->getRect().y()>(itsLevel[j][i]->getRect().y()-45) && itsPerson->getRect().y()<(itsLevel[j][i]->getRect().y()+90))
            {
                ...
                itsMenuFin = new ResultMenu(nullptr,itsTime,":/Textes/niveau2.txt");
                if(itsMenuFin->exec()==0)
                {
                    ...
                    {
                        TheGameLoop *aGameloop = new TheGameLoop(nullptr,":/Textes/niveau2.txt");
                        aGameloop->show();
                        itsTimer->stop();
